 ...
 */
AIPlayer1::AIPlayer1(GameBoard* _game, int _player, int _opponent, int _depth) : AIPlayer(_game, _player) { 
	opponent = _opponent;
	depth = _depth;
}
 ...
 *
 */
int AIPlayer1::rate_move(GameBoard* theGame, int x, int y, int thePlayer) const
{
    // greedy heuristic
...
int AIPlayer1::do_switch(int thePlayer) {
	if (thePlayer == player)
		return opponent;
	else
		return player;
...
bool AIPlayer1::is_move_candicate(GameBoard* theGame, int x, int y) {
...
int AIPlayer1::alphaBetaSearch(GameBoard* theGame, int alpha, int beta, int l, int thePlayer) {
...
void AIPlayer1::play() {
    ...
    int val = 0,
		...
		beta = 32767,
		alpha = -32767;
	std::vector<Pos> * moveList = new vector<Pos>();
