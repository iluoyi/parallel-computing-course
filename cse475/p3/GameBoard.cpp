...
/**
 * Construct by copying the given game board
 */
GameBoard::GameBoard(GameBoard* _gameBoard) : size(_gameBoard->size)
{
    assert(size > 2);

    score[0] = _gameBoard->get_score(0);
    score[1] = _gameBoard->get_score(1);
    score[2] = _gameBoard->get_score(2);
    status = _gameBoard->get_status();
    ...
    for (int row = 0; row < size; row++)
        for (int col = 0; col < size; col++)
            gameboard[row][col] = _gameBoard->gameboard[row][col];
}
